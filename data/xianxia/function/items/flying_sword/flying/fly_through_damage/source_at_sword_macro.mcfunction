...
$execute at @e[type=minecraft:item_display,tag=flying_sword_damage_source,limit=1] positioned ~ ~$(fly_thru_y_offset) ~ as @e[distance=..$(fly_thru_radius),tag=!fly_thru_dmg,tag=!flying_sword_fly_dmg_owner,type=!minecraft:item_display,type=!minecraft:block_display,type=!minecraft:text_display,type=!minecraft:marker,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:area_effect_cloud] at @s run function xianxia:items/flying_sword/flying/fly_through_damage/hit_macro with storage xianxia:items/flying_sword/config
